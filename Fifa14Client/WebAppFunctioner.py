...
class WebAppFunctioner(object):
    ...
    COIN_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/user/credits'
    TRANSFER_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/transfermarket?type=%s&lev=%s'\
                   '&pos=%s&num=%s&team=%s&macr=%s&micr=%s&minb=%s&nat=%s&maxb=%s'\
                   '&playStyle=%s&leaga=%s&start=%s&cat=%s&definitionId=%s&maskedDefId=%s'
    
    BID_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/trade/%s/bid'
    MOVE_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/item'
    UNASSIGNED_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/purchased/items'
    LIST_CARD_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/auctionhouse'
    TRADEPILE_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/tradepile'
    QUICKSELL_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/item/%s'
    WATCHLIST_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/watchlist'
    WATCHLIST_REMOVE_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/watchlist?tradeId=%s'
    TRADEPILE_REMOVE_URL = 'https://utas.s2.fut.ea.com/ut/game/fifa14/trade/%s'
    ...
    def move(self,card,pile):
        ...
        headers = {'X-UT-PHISHING-TOKEN':self.login_manager.FUTWEBPHISHING,
                   'X-UT-SID':self.login_manager.X_UT_SID,
                   'X-HTTP-Method-Override':'PUT'}
        payload = '{"itemData":[{"id":"%s","pile":"%s"}]}' % (card,pile)
    ...
    def list_card(self,card,starting_bid, buy_now_price, duration):
        """Lists card in transfer market for specified price and buy now price"""
        headers = {'X-UT-PHISHING-TOKEN':self.login_manager.FUTWEBPHISHING,
                   'X-UT-SID':self.login_manager.X_UT_SID,
                   'X-HTTP-Method-Override':'POST'}
        payload = '{"buyNowPrice":%s,"itemData":{"id":%s},"duration":%s,"startingBid":%s}' % (buy_now_price,card,duration,starting_bid) 
    ...
    def remove_from_tradepile(self,card):
        ...
        headers = {'X-UT-PHISHING-TOKEN':self.login_manager.FUTWEBPHISHING,
                   'X-UT-SID':self.login_manager.X_UT_SID,
                   'X-HTTP-Method-Override':'DELETE'}
        the_url = self.TRADEPILE_REMOVE_URL % (card)
        r = requests.post(the_url,headers = headers)
        return "code" not in dict(r.json())
